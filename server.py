...
ROLE_STATS = {
    "wizard": {
        ...
        "basic_cooldown": 0.4,
        "abilities": {
            "Q": {"name": "Arc Burst", "cooldown": 20},
            "E": {"name": "Veil Step", "cooldown": 40},
        },
    },
    "fighter": {
        "max_health": 125,
        ...
        "basic_cooldown": 0.52,
        "abilities": {
            "Q": {"name": "Seismic Slam", "cooldown": 35},
            "E": {"name": "Rift Slash", "cooldown": 40},
        },
    },
    "juggernaut": {
        "max_health": 1500,
        ...
        "basic_cooldown": 0.9,
        "abilities": {
            "Q": {"name": "Earthquake", "cooldown": 30},
            "E": {"name": "Hunter Leap", "cooldown": 50},
            "R": {"name": "Cyclone Maul", "cooldown": 45},
        },
    },
}
...
def create_room():
    ...
    room = {
        ...
        "last_tick": now(),
        "host_view": {
            "camera_x": ARENA["center_x"],
            "camera_y": ARENA["center_y"],
            "zoom": 0.58,
        },
    }
    ROOMS[code] = room
...
def visible_targets(room, source):
    ...
    for target in room["players"].values():
        if target["dead"] or target["id"] == source["id"]:
            continue
...
def use_ability(room, player, slot):
    if player["dead"]:
        return
    if player["cooldowns"].get(slot, 0) > 0:
        return
...
def update_camera(room):
    ...
    zoom_x = 1280 / max(spread_x, 1)
    zoom_y = 720 / max(spread_y, 1)
    room["host_view"]["zoom"] = clamp(min(zoom_x, zoom_y), 0.38, 0.9)
    ...
    while True:
        with ROOM_LOCK:
            ...
            for room in ROOMS.values():
                dt = min(1 / 20, current - room["last_tick"])
                room["last_tick"] = current
                if not room["started"]:
                    continue
                room["match_time"] += dt
...
def serialize_room_state(room, viewer_id=None):
    ...
    for player in room["players"].values():
        hidden_to_viewer = (
            viewer
            and viewer["role"] == "juggernaut"
            and player["role"] == "wizard"
            and player["invisible_until"] > room["match_time"]
            and player["id"] != viewer["id"]
        )
        players.append({
            "id": player["id"],
            "name": player["name"],
            "role": player["role"] or player["preferred_role"],
            "team": player["team"],
            "x": player["x"],
            "y": player["y"],
            ...
            "dead": player["dead"],
            "invisible": player["invisible_until"] > room["match_time"],
            "hiddenToViewer": hidden_to_viewer,
            "color": ROLE_COLORS.get(player["role"] or "wizard", "#9ae2ff"),
        })
    ...
    if viewer:
        ...
        abilities = []
        for key, meta in stats["abilities"].items():
            abilities.append({
                "key": key,
                "name": meta["name"],
                "cooldown": viewer["cooldowns"].get(key, 0.0),
            })
        viewer_payload = {
            "id": viewer["id"],
            "name": viewer["name"],
            "role": viewer["role"],
            "preferredRole": viewer["preferred_role"],
            "health": viewer["health"],
            "maxHealth": viewer["max_health"],
            "dead": viewer["dead"],
            "invisibleRemaining": max(0.0, viewer["invisible_until"] - room["match_time"]),
            "abilities": abilities,
        }
    ...
    return {
        ...
        "winner": room["winner"],
        "matchTime": room["match_time"],
        "arena": ARENA,
        "camera": room["host_view"],
...
class Handler(SimpleHTTPRequestHandler):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, directory=str(PUBLIC_ROOT if PUBLIC_ROOT.exists() else ROOT), **kwargs)
    ...
    def do_GET(self):
        ...
        if parsed.path == "/api/state":
            ...
            with ROOM_LOCK:
                room = ROOMS.get(code)
                if not room:
                    return self.write_json({"error": "Room not found."}, HTTPStatus.NOT_FOUND)
                return self.write_json(serialize_room_state(room, viewer_id or None))
        return super().do_GET()
    ...
    def do_POST(self):
        ...
        if parsed.path == "/api/create-room":
            with ROOM_LOCK:
                room = create_room()
                return self.write_json({"code": room["code"]})
        ...
        if parsed.path == "/api/join-room":
            ...
            with ROOM_LOCK:
                ...
                if not room:
                    return self.write_json({"error": "Room not found."}, HTTPStatus.NOT_FOUND)
                if room["started"]:
                    return self.write_json({"error": "Match already started."}, HTTPStatus.BAD_REQUEST)
                player = new_player(name)
                room["players"][player["id"]] = player
                room["message"] = f"{len(room['players'])} players in lobby"
                return self.write_json({"playerId": player["id"], "code": code})

        if parsed.path == "/api/set-role":
            code = str(payload.get("code", "")).upper()
        ...
        if parsed.path == "/api/restart-match":
            code = str(payload.get("code", "")).upper()
            with ROOM_LOCK:
                room = ROOMS.get(code)
                if not room:
...
if __name__ == "__main__":
    tick_thread = threading.Thread(target=tick_rooms, daemon=True)
    tick_thread.start()
    server = ThreadingHTTPServer(("0.0.0.0", PORT), Handler)
    print(f"Island Juggernaut server running at http://localhost:{PORT}")
